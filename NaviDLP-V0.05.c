...
// PORT: 6969
// CURRENT VERSION: V0.05 (INCOMPLETE) FINAL BITS OF ITERATION BEFORE GOING TO DEBUGGING
//
// USE DEPENDENCIES:
// cjson
// libmicrohttpd
...
// yt-dlp
//
// GLOBAL TODO: FINISH SEARCHSYS (NAVIDROME RESULT COMBINATION)
// GLOBAL PLAN:
...
if(pid==0){
	...
	snprintf(searchterm,sizeof(searchterm),"ytsearch5:%s",query);
		char *search[]={"yt-dlp",searchterm,"--dumbuilder-json","--no-download", NULL};
...
// PROXY SEND
static enum MHD_Result handle_proxy(struct MHD_Connection *connection, const char *url){
    struct proxyparams p = {"", 1};
    MHD_get_connection_values(connection, MHD_GET_ARGUMENT_KIND, buildquery, &p);
    char curlproxyargs [4096];
