...
// PORT: 6969
// CURRENT VERSION: V0.07 (INCOMPLETE) FINAL BITS OF ITERATION BEFORE GOING TO DEBUGGING
//
// USE DEPENDENCIES:
// cjson
// libmicrohttpd
...
// yt-dlp
//
// GLOBAL TODO: FINAL ERRORCHECK BEFORE DEBUG
// GLOBAL PLAN:
// ESTIMATE TO STABLE: 86%
...
static enum MHD_Result buildquery(void *cls, enum MHD_ValueKind kind, const char *key, const char *value);
static enum MHD_Result Jameswilliam(struct MHD_Connection *connection, const char *url);
...
if(strcmp(url,"/rest/search3.view")==0){
	...
	return Jameswilliam (connection,url);
}
...
struct membuf NAVISEARCHSYS = {0};
CURL *navisearch = curl_easy_init();
	curl_easy_setopt(navisearch, CURLOPT_WRITEFUNCTION, writecb);
	curl_easy_setopt(navisearch, CURLOPT_WRITEDATA, &NAVISEARCHSYS);
...
// PARSE NAVIDROME RESULTS
cJSON *naviparse = cJSON_Parse(NAVISEARCHSYS.data);
	if(naviparse == NULL) goto compile;
		cJSON *navisubres = cJSON_GetObjectItem(naviparse, "subsonic-response");
		cJSON *navisearchresult = cJSON_GetObjectItem(navisubres, "searchResult3");
		cJSON *navisongs = cJSON_GetObjectItem(navisearchresult, "song");
		if(navisubres==NULL || navisearchresult==NULL || navisongs==NULL) goto compile;
	int navicount = cJSON_GetArraySize(navisongs);
	for(int index=0; index<navicount; index++){
    	cJSON *navisong = cJSON_GetArrayItem(navisongs,index);
    	cJSON_AddItemToArray(songarray, cJSON_Duplicate(navisong, 1));
}
cJSON_Delete(naviparse);
...
// SEND STREAM
	struct MHD_Response *availstreamresp = MHD_create_response_from_buffer(availstream.size,availstream.data,MHD_RESPMEM_MUST_FREE);
...
CURL *naviscan = curl_easy_init();
	curl_easy_setopt(naviscan,CURLOPT_URL,"http://localhost:4533/rest/startScan.view");
...
// BUILD THE STREAM RESPONSE
	cJSON *getid = cJSON_Parse(IDGRABRESP.data);
...
// PLAN: 
//
struct proxyparams { char fquery[4096]; int first; };
static enum MHD_Result buildquery(void *cls, enum MHD_ValueKind kind, const char *key, const char *value){
    struct proxyparams *builder = cls;
    if(builder->first){ strncat(builder->fquery, "?", sizeof(builder->fquery)-strlen(builder->fquery)-1); builder->first=0; }
    else strncat(builder->fquery, "&", sizeof(builder->fquery)-strlen(builder->fquery)-1);
    strncat(builder->fquery, key, sizeof(builder->fquery)-strlen(builder->fquery)-1);
    strncat(builder->fquery, "=", sizeof(builder->fquery)-strlen(builder->fquery)-1);
    strncat(builder->fquery, value, sizeof(builder->fquery)-strlen(builder->fquery)-1);
    return MHD_YES;
}
// PROXY SEND
static enum MHD_Result Jameswilliam(struct MHD_Connection *connection, const char *url){
    struct proxyparams p = {"", 1};
    MHD_get_connection_values(connection, MHD_GET_ARGUMENT_KIND, buildquery, &p);
    char curlproxyargs [8888]; // chinese lucky numbers to hopefully bless up this clusterfuck lmfaooo
    snprintf(curlproxyargs, sizeof(curlproxyargs), "http://localhost:4533%s%s", url, p.fquery);
    struct membuf curlproxy = {0};
    CURL *curlproxydo = curl_easy_init();
    curl_easy_setopt(curlproxydo, CURLOPT_WRITEFUNCTION, writecb);
    curl_easy_setopt(curlproxydo, CURLOPT_WRITEDATA, &curlproxy);
    curl_easy_setopt(curlproxydo, CURLOPT_URL,curlproxyargs);
    curl_easy_perform(curlproxydo);
    curl_easy_cleanup(curlproxydo);
    struct MHD_Response *proxyresp = MHD_create_response_from_buffer(curlproxy.size, curlproxy.data, MHD_RESPMEM_MUST_FREE);
    MHD_queue_response(connection, MHD_HTTP_OK, proxyresp);
    MHD_destroy_response(proxyresp);
    return MHD_YES;
}